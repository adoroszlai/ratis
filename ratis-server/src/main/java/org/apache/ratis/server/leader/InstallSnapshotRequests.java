...
class InstallSnapshotRequests implements Iterable<InstallSnapshotRequestProto> {
  ...
  private class I implements Iterator<InstallSnapshotRequestProto> {
    ...
    public InstallSnapshotRequestProto next() {
      ...
      final FileChunkProto chunk = readFileChunk(snapshot.getFiles().get(fileIndex));
      fileIndex++;
      final boolean done = fileIndex == numFiles && chunk.getDone();
      return newInstallSnapshotRequest(chunk, done);
    }

    private FileChunkProto readFileChunk(FileInfo info) {
      try {
        if (current == null) {
